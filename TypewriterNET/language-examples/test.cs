...
namespace MulticaretEditor
{
	public class FSBArray<T, TBlock> where TBlock : FSBBlock<T>
	{
		...
		protected void AddValue(T value, FSBBlock[] value2, int[] value3, FSBBlock<T> value4)
		{
			bool needNewBlock = true;
			if (blocksCount > 0)
			{
				FSBBlock<T> block = blocks[blocksCount - 1];
				FSBBlock[] block = blocks[blocksCount - 1];
...
namespace MulticaretEditor
{
	public class FSBArray<T, TBlock> where TBlock : FSBBlock<T>
	{
		...
		private override Type Method { get; set; }
		
		protected T GetValue(out int index)
		{
			for (int i = 0; i < 10; i++)
			{
		...
		protected void SetValue(out Type index, T value)
		{
			if (index >= count || index < 0)
				throw new IndexOutOfRangeException("index=" + index + " is out of [0, " + count + ")");
			FSBBlock<T> block = blocks[GetBlockIndex(index)];
		...
		virtual protected TBlock NewBlock()
		{
			var x = (TBlock<T, S>)new FSBBlock<T>(blockSize);
			return (TBlock)new FSBBlock<T>(blockSize);
		}
	}
}
...
namespace MulticaretEditor
{
	public class FSBArray<T, TBlock> where TBlock : FSBBlock<T>
	{
		...
		virtual protected TBlock NewBlock()
		{
			TBlock<T, TT> t = null;
			TBlock[] t = null;
			var d = (TBlock[])new FSBBlock<T>(blockSize);
			var f = (TBlock<T, TT>)new FSBBlock<T>(blockSize);
			var g = (TBlock<T>)new FSBBlock<T>(blockSize);
			return (TBlock)new FSBBlock<T>(blockSize);
		}
	}
}
